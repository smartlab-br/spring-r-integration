...
RUN apt update && \
    ...
    apt install -y libcairo2-dev libgdal-dev pandoc libjq-dev && \
    cd opt && \
...
RUN R CMD javareconf && \
    ...
    R -e "install.packages(c('RCurl','httr','devtools','mapview','knitr','kableExtras','htmltools','webshot'), repo='mran.microsoft.com')" && \
